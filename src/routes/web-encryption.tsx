...
const project = getProjectBySlug("web-encryption")!;

export const Route = createFileRoute("/web-encryption")({
  head: () => ({
    meta: [
      { title: "Website Encryption — Projects LUX" },
      { name: "description", content: project.summary },
    ],
  }),
  component: WebEncryptionPage,
});

function WebEncryptionPage() {
  return (
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-[1.1fr_0.9fr] gap-8 items-start">
          <section>
            ...
            <div className="grid md:grid-cols-3 gap-4 mt-10">
              ...
              <Meta label="Partner Note" value={project.partnerNote ?? "—"} />
            </div>

            <div className="mt-12 space-y-10">
              <Block title="ليش الأداة مفيدة؟" items={project.whyItMatters} />
              <Block title="المميزات" items={project.features} />
              <Block title="من صممها؟" items={[project.designedBy, project.partnerNote ?? "شركاء متخصصون في نفس المجال", `الحالة الحالية: ${projectStatusArabic[project.status]}`]} />
            </div>
          </section>
          ...
          <aside className="space-y-5 lg:sticky lg:top-28">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
              ...
              <p dir="rtl" className="font-arabic mt-3 text-sm leading-[1.95] text-foreground/85">
                الأداة حالياً ضمن المرحلة {projectStatusArabic[project.status]}، ومهيأة للتحول إلى منتج فعلي عندما يتم بناء الواجهة والأذونات ونظام الاستخدام.
              </p>
              <div className="mt-5 grid gap-3">
                <button className="inline-flex items-center justify-center rounded-full bg-foreground text-background h-11 text-[11px] tracking-[0.24em] uppercase font-semibold hover:opacity-90 transition">
                  ابدأ الآن
                </button>
                <Link
                  to="/projects"
